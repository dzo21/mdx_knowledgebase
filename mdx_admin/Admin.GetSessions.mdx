-- This query provides an overview of current sessions in SQL Server Analysis Services (SSAS),
-- including detailed session information like session ID, user name, current database, memory usage,
-- command count, elapsed time, CPU time, and read/write statistics.
-- It is designed to help identify the busiest sessions by ordering the results by CPU time used.
